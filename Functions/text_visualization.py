    ...
    **This class only supports file-based input (TXT, DOCX, PDF).**
    ...
    def __init__(self, file_path):
        ...
        if not isinstance(file_path, str) or not os.path.exists(file_path):
            raise ValueError("Error: Text visualization only supports file-based input. Provide a valid file path.")

        # Initialize processing classes
        self.basic = Basic(file_path)
        self.advanced = Advanced(file_path)
        self.sentiment = Sentiment(self.basic.text)  # Sentiment analysis on extracted text
        ...
        self.output_dirs = {
            "wordcloud": f"visualizations/wordclouds/{self.filename}",
            "freqplot": f"visualizations/frequency_plots/{self.filename}",
            "sentiment": f"visualizations/sentiment_graphs/{self.filename}",
            "tfidf": f"visualizations/tfidf_heatmaps/{self.filename}",
        }
    ...
    def generate_word_cloud(self):
        ...
        if not text:
            raise ValueError("Error: No text found in the file for word cloud generation.")
    ...
    def generate_word_frequency_plot(self):
        ...
        if not filtered_tokens:
            raise ValueError("Error: No valid words found in the file for frequency plot.")
        ...
        for bar in bars:
            plt.text(bar.get_x() + bar.get_width()/2., bar.get_height(), str(int(bar.get_height())), ha="center", fontsize=8)
    ...
    def generate_sentiment_distribution(self):
        ...
        sizes = [sentiment_results["polarity"] * 100 if sentiment_results["polarity"] > 0 else 0,
                 abs(sentiment_results["polarity"]) * 100 if sentiment_results["polarity"] < 0 else 0,
        ...
        output_path = self._get_output_path("sentiment", f"{self.filename}_sentiment.jpg")
        plt.savefig(output_path, bbox_inches='tight', dpi=300)
        plt.close()
    ...
    def generate_tfidf_heatmap(self):
        ...
        df = pd.DataFrame({'Word': tfidf_results["Top TF-IDF Words"], 'TF-IDF Score': tfidf_results["TF-IDF Scores"]})
        df = df.set_index('Word')

        plt.figure(figsize=(12, 8))
        cmap = LinearSegmentedColormap.from_list('blue_gradient', ['#EBF5FB', '#2471A3'])
